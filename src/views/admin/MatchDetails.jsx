...
import { Mic, Activity, Circle, Share2, Check, User, Crown } from 'lucide-react'; // Added User, Crown icons
...
export default function MatchDetails({ currentMatch, setView }) {
  ...
  const getFormattedTime = () => {
      ...
      const date = currentMatch.timestamp.seconds 
        ? new Date(currentMatch.timestamp.seconds * 1000) 
        : new Date(currentMatch.timestamp);
      return date.toLocaleString([], { dateStyle: 'medium', timeStyle: 'short' });
  };

  // --- HELPER: Calculate Ball Number for Timeline ---
  const getBallNumber = (index) => {
     let count = parseInt(currentMatch.legalBalls || 0);
     for(let i = 0; i < index; i++) {
         if(currentMatch.timeline[i].type === 'legal') count--;
     }
     return formatOvers(count);
  };

  // --- HELPER: Get Stats Safely ---
    ...
    <div className="space-y-6 pb-20">
      <div className="bg-white rounded-2xl shadow-xl overflow-hidden border border-gray-100">
        <div className="bg-gray-900 text-white p-4 md:p-6 relative">
           ...
           <button 
             onClick={shareMatch}
             className="md:hidden absolute top-4 right-4 p-2 bg-white/10 rounded-full hover:bg-white/20 transition-colors z-10"
           ...
           <div className="flex flex-col md:flex-row justify-between items-center gap-4 mb-6 pt-2 md:pt-0">
              ...
              <div className="flex items-center gap-3 w-full md:w-auto justify-center md:justify-start">
                 ...
                 <span className="font-bold opacity-80 text-lg md:text-xl tracking-wide truncate">{currentMatch.teamA}</span>
              </div>

              {/* Status Badge */}
              <div className="my-2 md:my-0 transform scale-90 md:scale-100 order-first md:order-none">
              ...
              <div className="flex items-center justify-center md:justify-end w-full md:w-auto gap-4">
                  ...
                  <button 
                    onClick={shareMatch}
                    className="hidden md:flex items-center justify-center p-2 bg-white/10 rounded-full hover:bg-white/20 transition-colors ml-2"
                    title="Copy Link"
                  >
           ...
           <div className="text-center mb-6">
              ...
              <div className="flex flex-wrap justify-center items-center gap-x-4 gap-y-2 text-sm font-medium text-gray-400">
                 <span>Overs: {formatOvers(currentMatch.legalBalls)} / {currentMatch.totalOvers}</span>
                 <span>CRR: {calculateRunRate(currentMatch.score, currentMatch.legalBalls)}</span>
                 <span className="text-green-400">Extras: {currentMatch.extras || 0}</span>
              </div>
               ...
               <div className="border-t border-gray-800 pt-4 mt-4 space-y-4">
                   ...
                   <div>
                       ...
                       {currentMatch.striker && (
                           <div className="grid grid-cols-12 gap-1 md:gap-2 text-xs md:text-sm items-center mb-1">
                                <div className="col-span-5 text-white font-bold flex items-center">
                                    <span className="truncate">{currentMatch.striker}</span>
                                    <Circle className="w-2 h-2 md:w-2.5 md:h-2.5 fill-green-500 text-green-500 ml-2 shrink-0 animate-pulse" />
                                </div>
                                <div className="col-span-2 text-right font-bold text-white">{strikerStats.runs}</div>
                                <div className="col-span-2 text-right">{strikerStats.balls}</div>
                                <div className="col-span-1 text-right text-gray-400">{strikerStats.fours}</div>
                                <div className="col-span-1 text-right text-gray-400">{strikerStats.sixes}</div>
                                <div className="col-span-1 text-right text-gray-400">{strikerStats.sr}</div>
                           </div>
                       )}
                       ...
                       {currentMatch.nonStriker && (
                           <div className="grid grid-cols-12 gap-1 md:gap-2 text-xs md:text-sm items-center">
                                ...
                                <div className="col-span-2 text-right font-bold">{nonStrikerStats.runs}</div>
                                <div className="col-span-2 text-right">{nonStrikerStats.balls}</div>
                                <div className="col-span-1 text-right text-gray-500">{nonStrikerStats.fours}</div>
                                <div className="col-span-1 text-right text-gray-500">{nonStrikerStats.sixes}</div>
                                <div className="col-span-1 text-right text-gray-500">{nonStrikerStats.sr}</div>
                           </div>
                       )}
                   </div>
                   ...
                   <div>
                       ...
                       {currentMatch.bowler && (
                           <div className="grid grid-cols-12 gap-1 md:gap-2 text-xs md:text-sm items-center">
                                ...
                                <div className="col-span-2 text-right">{bowlerStats.oversDisplay}</div>
                                <div className="col-span-1 text-right text-gray-400">0</div>
                                <div className="col-span-2 text-right">{bowlerStats.runs}</div>
                                <div className="col-span-1 text-right font-bold text-white">{bowlerStats.wickets}</div>
                                <div className="col-span-1 text-right text-gray-400">{bowlerStats.eco}</div>
                           </div>
                       )}
                   </div>
                   ...
                   {currentMatch.timeline && currentMatch.timeline.length > 0 && (
                       <div className="bg-gray-800/60 rounded-full px-4 py-3 text-xs text-gray-300 flex items-center justify-center italic border border-gray-700 mt-4 text-center">
                           <Mic className="w-3 h-3 mr-2 text-green-500 shrink-0" />
                           <span>{getLastActionText(currentMatch)}</span>
                       </div>
                   )}
               </div>
           )}
        ...
        {activeTab === 'timeline' && (
            <div className="p-4 bg-gray-50 space-y-2">
                ...
                {(!currentMatch.timeline || currentMatch.timeline.length === 0) && (
                    <div className="text-center text-gray-400 p-4 italic">No balls bowled yet.</div>
                )}
                {currentMatch.timeline?.map((ball, index) => (
                    <div key={index} className="bg-white p-3 rounded-lg shadow-sm border border-gray-100 flex justify-between items-center">
                        <div className="flex items-center gap-3">
                            ...
                            <div className={`w-8 h-8 rounded-full flex items-center justify-center font-bold text-xs shrink-0 ${ball.isWicket ? 'bg-red-500 text-white' : ball.runs === 4 ? 'bg-blue-500 text-white' : ball.runs === 6 ? 'bg-purple-600 text-white' : 'bg-gray-100 text-gray-700'}`}>
                                {ball.isWicket ? 'W' : ball.type !== 'legal' ? ball.type.toUpperCase() : ball.runs}
                            </div>
                            <div className="text-sm">
                                <div className="font-semibold text-gray-800">{ball.bowler} to {ball.striker}</div>
                                <div className="text-xs text-gray-500">
                                    {ball.isWicket ? `OUT (${ball.dismissalInfo?.type || 'Wicket'})` : ball.type === 'wide' ? 'Wide Ball' : ball.type === 'nb' ? 'No Ball' : `${ball.runs} Run${ball.runs !== 1 ? 's' : ''}`}
                                </div>
                            </div>
                        </div>
                        <div className="text-[10px] text-gray-400 shrink-0">
                           {new Date(ball.timestamp).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'})}
                        </div>
                    </div>
                ))}
            </div>
        ...
        {/* --- INFO TAB (UPDATED with Player Cards) --- */}
        {activeTab === 'info' && (
           <div className="p-4 bg-gray-50 space-y-4">
              <div className="bg-white p-4 rounded-lg shadow-sm border border-gray-100">
                 ...
                 <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                    ...
                    <div className="md:col-span-2"><p className="text-gray-400 text-xs uppercase font-bold">Toss</p><p className="font-semibold text-gray-800 mt-1">{currentMatch.innings1 ? `${currentMatch.innings1.teamName} batted first` : `${currentMatch.battingTeam} elected to bat`}</p></div>
                 </div>
              </div>
